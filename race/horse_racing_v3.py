...
race_ids = sorted(data['Id'].unique())
logger.info(f"Found {len(race_ids)} unique race IDs")

# Create a dictionary to map race IDs to dates
race_date_map = data.groupby('Id')['date'].first().to_dict()
date_list = [race_date_map.get(id, pd.NaT) for id in race_ids]
result_data = {}
