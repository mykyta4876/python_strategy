    ...
    def get_my_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error decoding JSON from GetMyData response: {e}\nResponse text: {response.text}")
                return []
...
def test():
    ...
    df = downloader.parse_betfair_data("betfair_data\\betfair_data_1")
    print(df.head())
    print(f"\nData shape: {df.shape}")
    print(f"Columns: {list(df.columns)}")

    # Save to CSV
    df.to_csv("betfair_data\\betfair_data_1_parsed.csv", index=False)
    print(f"\nParsed data saved to: betfair_data\\betfair_data_1_parsed.csv")
