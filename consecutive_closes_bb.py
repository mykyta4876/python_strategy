...
def calculate_bollinger_bands(df, window, num_std_dev):
    ...
    df['BB_Upper'] = df['MA'] + (df['Close'].rolling(window=window).std() * num_std_dev)
    return df

def simulate_trades(market_data, initial_assets=10000, num_long_entry=3, num_long_exit=2, bollinger_window=20, bollinger_std_dev=1.5):
    balance = initial_assets
    trades = []

    for market, df in market_data.items():
        df = calculate_bollinger_bands(df, bollinger_window, bollinger_std_dev)
        df['LongEntry'] = check_consecutive_closes(df, num_long_entry, direction='positive') & (df['Close'] > df['BB_Upper'])
...
def plot_results(market_data, trades):
    ...
    for market, df in market_data.items():
        ...
        ax1.plot(df.index, df['BB_Upper'], linestyle='--', label=f'{market} Bollinger Band')

    labels_added = {'Long Entry': False, 'Long Exit': False}
    for trade in trades:
        label = None
        if 'Entry' in trade[2]:
            if 'Long' in trade[2]:
...
def parse_arguments():
    ...
    parser.add_argument("--bollinger_std_dev", "-bsd", type=float, default=1.5, help="Standard deviation for Bollinger Bands")
...
def main():
    ...
    trades, final_balance = simulate_trades(market_data, 
                                            ...
                                            bollinger_std_dev=args.bollinger_std_dev)
