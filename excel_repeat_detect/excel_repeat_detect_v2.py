...
records = defaultdict(lambda: {'count': 0, 'files': [0] * len(excel_files), 'data': set()})
...
for key, value in records.items():
    ...
    data = value['data']  # Join all unique "Data" values as a string
    result_data.append([date_time, team, data, count] + file_flags)

# Define the columns for the result DataFrame
columns = ['Date Time', 'Team', 'Data', 'Repeat Count'] + excel_files
